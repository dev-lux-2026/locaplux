...
export default function PartnerOrderDetailsPage() {
  ...
  // 🔥 Charger la commande partenaire
  useEffect(() => {
    fetch(`/api/partner/orders/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setOrder(data);
        setLoading(false);
  ...
  async function markDelivered() {
    ...
    const res = await fetch(`/api/partner/orders/${id}/delivery`, {
      method: "PATCH",
      ...
      <div className="max-w-4xl mx-auto py-12 px-4 lg:px-0 space-y-10">
        ...
        <header>
          ...
          <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">
            Détails complets de la commande et gestion de la livraison.
          </p>
        </header>

        {/* LIVRAISON */}
        <section className="bg-white dark:bg-[#0F0F10] border border-gray-200 dark:border-white/10 rounded-2xl p-6 space-y-4 shadow-sm">
          ...
          {order.deliveryStatus !== "delivered" && (
            <button
              ...
              disabled={updating}
              className="mt-4 px-5 py-2 bg-green-600 text-white rounded-xl hover:bg-green-700 transition disabled:opacity-50"
            >
              Marquer comme livrée
            </button>
          )}
        ...
        {/* ADRESSE */}
        <section className="bg-white dark:bg-[#0F0F10] border border-gray-200 dark:border-white/10 rounded-2xl p-6 space-y-2 shadow-sm">
        ...
        {/* ARTICLES */}
        <section className="bg-white dark:bg-[#0F0F10] border border-gray-200 dark:border-white/10 rounded-2xl p-6 space-y-6 shadow-sm">
